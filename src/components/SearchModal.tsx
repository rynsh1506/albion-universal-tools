  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center justify-between gap-4 bg-base-content/5 border border-base-content/10 hover:border-primary/50 hover:bg-primary/5 px-5 py-3 rounded-2xl text-xs font-black text-base-content transition-all min-w-37.5 shadow-sm active:scale-95"
      >
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-[calc(100%+8px)] left-0 w-full bg-base-200/95 backdrop-blur-2xl border border-base-content/10 rounded-2xl shadow-2xl z-50 overflow-hidden py-2"
            >
              <div className="max-h-60 overflow-y-auto custom-scrollbar">
                {["All", ...options.map((o: any) => o.val)].map((optVal, i) => (
                  <button
                    ...
                    className={`w-full text-left px-5 py-3 text-xs font-black uppercase tracking-widest hover:bg-primary/20 transition-colors ${value === optVal ? "text-primary bg-primary/5" : "text-base-content/60"}`}
                  >
                    {optVal === "All"
                      ? `All ${label}`
...
const FallbackImage = ({ src, alt, itemName, isSelected }: any) => {
  ...
  if (imgFailed || !src) {
    ...
    const iconClass = `w-14 h-14 opacity-50 drop-shadow-md ${isSelected ? "text-primary-content" : "text-base-content"}`;
    return (
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="relative bg-base-200/95 backdrop-blur-2xl w-full max-w-4xl h-[80vh] rounded-4xl border border-base-content/10 shadow-[0_0_80px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col"
          ...
          >
            ...
            <div className="px-8 py-6 flex items-center justify-between relative z-20">
              ...
              <button
                onClick={handleReset}
                className="btn bg-error/10 hover:bg-error/20 text-error rounded-2xl px-6 h-12 flex items-center gap-2 text-xs font-black uppercase transition-all shadow-sm active:scale-90"
              >
            ...
            <div className="flex-1 flex flex-col bg-base-content/5 mx-8 mb-4 rounded-3xl overflow-hidden border border-base-content/5">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {paginatedItems.map((item, index) => (
                        <div
                          ...
                          className={`px-6 py-4 rounded-2xl cursor-pointer flex items-center justify-between transition-colors duration-100 ${index === selectedIndex ? "bg-primary text-primary-content shadow-xl" : "hover:bg-base-content/5"}`}
                        >
              ...
              {totalPages > 1 && (
                <div className="px-8 py-3 border-t border-base-content/5 flex items-center justify-between bg-base-200/30 shrink-0">
                  <button
                    onClick={() =>
                      s.setSearchPage(Math.max(1, s.searchPage - 1))
                    }
                    disabled={s.searchPage === 1}
                    className="btn btn-ghost btn-xs font-black uppercase disabled:opacity-10"
                  >
            ...
            <div className="px-10 py-5 border-t border-base-content/5 flex justify-between items-center text-[10px] font-black uppercase opacity-20 select-none shrink-0">
              <div className="flex gap-8">
                ...
                <span>
                  <kbd className="kbd kbd-xs">←→</kbd> Pages
                </span>
                <span>
